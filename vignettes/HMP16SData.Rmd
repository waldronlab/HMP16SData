...
methods to analyze microbiome data. The `as_phyloseq` function coerces the
`SummarizedExperiment` class objects provided by
`r BiocStyle::Biocpkg("phyloseq")` to `phyloseq` class objects containing count
data, sample (participant) data, taxonomy, and phylogenetic tree:
...
To demonstrate, we first remove taxa with no counts in any sample to avoid
warning messages:
...
## Single Flat File Representation

Using the `r BiocStyle::CRANpkg("haven")` package it is possible to export data
from `r BiocStyle::Biocpkg("HMP16SData")` to other formats, for analysis beyond
the Bioconductor ecosystem. Examples of how to create a single flat file of the
16S variable region 1–3 data follow.
...
## Three Seperate Tables Representation

The `SummarizedExperiment` class objects provided by
`r BiocStyle::Biocpkg("HMP16SData")` are 
[normalized](https://tinyurl.com/7j6hpov) to provide the most storage efficent
representation of the data. It is best to keep taxonomic counts, taxonomic
classifications, and participant data stored as three seperate tables.
