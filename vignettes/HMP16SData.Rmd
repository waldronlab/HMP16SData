...
information about the protected metadata available and obtaining a key.
The `attach_dbGap()` function takes a `r BiocStyle::Biocpkg("HMP16SData")`
`SummarizedExperiment` object as its first argument and the path to your dbGaP
repository key as its second argument, and it returns another
...
abundance plot, the `phyloseq` class objects are _"melted"_ into `data.frame`
...
that explicitly for mapping to the WMS samples and the `SRS` identifiers are in
the `NCBI_accession` column of the WMS sample data. The intersection of the two
vectors represents the matching samples that are present in both data sets. This
intersection, shown below as `SRS_SAMPLE_IDS`, can then be used to filter both
the 16S and WMS samples to include only the samples in common.
...
Wong, B. Points of view: Color blindness. _Nat. Methods_ *8*, 441 (2011).
...
Finally, the `r BiocStyle::CRANpkg("gridExtra")` package is used to arrange,
scale, and plot the three plots in a single figure.
